...
$pathways = Get-Content -Raw -LiteralPath (Join-Path $root 'pathways.html')
$homepageScript = Get-Content -Raw -LiteralPath (Join-Path $root 'homepage.js')
$pathwaysScript = Get-Content -Raw -LiteralPath (Join-Path $root 'script.js')
...
foreach ($page in @($homepage, $pathways)) {
...
Assert-Dropdown ($pathways -match 'id="lecheng"') 'Lecheng anchor is missing.'
Assert-Dropdown ($pathways -match 'id="greater-bay-area"') 'Greater Bay Area anchor is missing.'
Assert-Dropdown ($pathways -match 'id="pathway-comparison"') 'Comparison anchor is missing.'
Assert-Dropdown ($homepageScript -match 'data-nav-dropdown-toggle') 'Homepage dropdown behavior is missing.'
Assert-Dropdown ($pathwaysScript -match 'data-nav-dropdown-toggle') 'Inner-page dropdown behavior is missing.'
Assert-Dropdown ($pathwaysScript -match 'scrollToRenderedAnchor') 'Post-render pathway anchor correction is missing.'
...
Write-Output 'Special Pathways dropdown checks passed (22 assertions).'
